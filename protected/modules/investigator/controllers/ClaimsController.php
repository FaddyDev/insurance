...
class ClaimsController extends Controller
{
	...
	{
		$model=$this->loadModel($id);
		/*if($model->claim_status == 'Closed')
		{
			$_SESSION['status']='fail'; $_SESSION['response']='Cannot comment on this claimed 
		...
		if(isset($_POST['Claims']))
		{
			...
			//for saving
			$model->attributes=$_POST['Claims'];
			$model->claim_investigator_post_datetime = (new DateTime())->format('Y-m-d H:i:s');
			$model->claim_status = 'Pending final verdict';
			//more for email
			$report = $model->claim_investigator_report;
			$status = $model->claim_status;
			if($model->save()){
				//$this->redirect(array('view','id'=>$model->pk_claim_id));{
				//email client				
				$message = 'Dear '.$recipientName.',<br><p>The status for your claim of ID <strong>'.$id.'</strong> has changed. 
				The investigator\'s report is: <em>'.$report.'</em> while the status is: <strong>'.$status.'</strong>.
				...
				$_SESSION['response']='Report updated successfully!';}
				else{$_SESSION['status']='fail'; 
					$_SESSION['response']='Report updated successfully, but there were errors sending the emal update to client!';}
			}
		}
	...
	{
		$datainvestigator=new CActiveDatainvestigator('Claims');
		$this->render('index',array(
			'datainvestigator'=>$datainvestigator,
		));
	}
